...
export function topicLessonSystemPrompt(language: AiLanguage): string {
  return [
    ...
    'Создай ПОЛНОЦЕННЫЙ учебный урок по одной теме — как самостоятельное занятие на десятки экранов чтения, а не краткую шпаргалку и не один длинный конспект.',
    'Главный контент верни в pages: отдельные учебные страницы/главы, каждая со своей целью, подробным объяснением, примерами и закреплением. Ученик должен читать урок по шагам, страницу за страницей.',
    'Каждая страница должна по-настоящему обучать: давай интуицию («почему это так»), выводи или обосновывай формулы, показывай как рассуждать, приводи маленькие пояснения прямо в тексте. Пиши развёрнуто, в 3–6 абзацев на страницу, без «воды», но и без излишней краткости.',
    'Иди от простого к сложному: сначала база и определения, затем методы и формулы, затем сложные случаи, типичные задания ЕНТ и связки с другими темами. Подсвечивай типичные заблуждения прямо по ходу объяснения.',
  ...
  const schema = `Схема ответа (JSON):
{
  ...
  "pages": [
    {
      "slug": "string — короткий латинский slug страницы",
      "title": "string — заголовок страницы",
      "goal": "string — микроцель этой страницы",
      "content": "string — развёрнутое объяснение на 3-6 абзацев, можно использовать LaTeX вида $...$ или $$...$$",
      "examples": [
        {
          "title": "string",
          "question": "string",
  ...
  "sections": [
    {
      "title": "string",
      "content": "string — краткое резюме ключевого раздела, можно использовать LaTeX вида $...$ или $$...$$"
    }
  ],
  "formulas": [
  ...
  return [
    'Создай полноценный урок для изучения темы:',
    '```json',
    JSON.stringify(data, null, 2),
    '```',
    '',
    'Требования к полноценному уроку:',
    '- 8-12 pages: отдельные страницы урока, не одна простыня. Каждая страница: микроцель, 3-6 абзацев объяснения, 1-2 разобранных примера или задания для закрепления, короткий checklist.',
    '- В pages покрой полный путь: вводная идея → определения → базовый алгоритм → формулы/правила → типовые задания ЕНТ → сложные случаи → ловушки → итоговая стратегия.',
    '- 5-7 sections как краткое резюме урока для быстрых якорей/оглавления, не основной контент.',
    ...
    '- 8-12 заданий для закрепления (practice) с вариантами и разбором в explanation, и 5-7 заданий в miniTest.',
    ...
    '- Будь содержательным и глубоким. Не обрывай JSON: лучше сделать 8 качественных страниц, чем 12 незавершённых.',
    '',
    schema,
  ].join('\n');
}
